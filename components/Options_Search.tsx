...
const Options_Search: React.FC<Props> = ({setLoading, setShowSelectedFilters, setFilterSelected}) => {
  const [showFilters, setShowFilters] = useState(false);
  const [valueQuery, setValueQuery] = useState<string | null>(null);
  const { diet, excludeIngredients, includeIngredients, clearFilters } = filterStore();
  ...
  const handleSearchWithFilters = async (
    ...
    if(!valueQuery && diet.length == 0 && includeIngredients.length == 0 && excludeIngredients.length == 0){
    ...
    if(includeIngredients.length != 0) request.includeIngredients = includeIngredients.join(',')
    if(excludeIngredients.length != 0) request.excludeIngredients = excludeIngredients.join(',')
  ...
  return (
    <>
      {/* SEARCHBAR */}
      <form className="row-start-2 place-content-center desktop:col-start-2 flex items-center justify-center gap-4" onSubmit={(evt) => handleSearchWithFilters(evt)}>
        <div
          className="bg-gray-300 w-[95%] max-w-[600px] rounded-full flex gap-2 items-center px-4 desktop:mx-auto desktop:w-[75%]"
        >
          <svg
        ...
        <button className="hidden bg-black text-white py-4  rounded-full desktop:flex items-center justify-center flex-1 max-w-[200px]" type="submit">
          Search
        </button>
      </form>
      {/* FILTER */}
      <div className="place-content-center desktop:row-span-2 desktop:col-start-1 desktop:ow-start-2 desktop:place-content-start flex items-center justify-between sm:justify-around desktop:items-start">
        ...
        <form onSubmit={(evt) => handleSearchWithFilters(evt)}>
          <button className="bg-black text-white w-[45px] h-[45px] rounded-full flex items-center justify-center desktop:hidden" type="submit">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
